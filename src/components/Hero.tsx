...
import AirflowGauge from './AirflowGauge';
  ...
  return (
    <section id="top" className="relative pt-32 pb-20 md:pt-44 md:pb-28 overflow-hidden grid-texture">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-10 grid md:grid-cols-2 gap-16 items-center">
        ...
        <div className="relative aspect-square max-w-xl mx-auto w-full rounded-3xl overflow-hidden border border-line">
          {/* Reference photo only — swap for licensed/owned photography before launch */}
          <img
            src="https://loremflickr.com/800/800/hvac,rooftop,newyork"
            alt="Reference photo of HVAC rooftop equipment"
            className="absolute inset-0 w-full h-full object-cover"
            loading="lazy"
          />
          <div className="absolute inset-0 bg-gradient-to-t from-navy-deep via-navy-deep/30 to-navy-deep/10" />
          <div className="absolute inset-0 grid-texture opacity-30" />
          <div className="absolute w-40 h-40 md:w-52 md:h-52 bottom-6 right-6 opacity-90">
            <AirflowGauge />
          </div>
          <span className="absolute top-4 left-4 text-[10px] font-mono uppercase tracking-wide text-ink/70 bg-navy-deep/70 backdrop-blur-sm px-2.5 py-1 rounded-full">
            Reference photo
          </span>
        </div>
      </div>
